...
## Changes in V2.0:    1) This script concats "Sample.canon_annotated_flagged.HRR" instead of "Sample.canon_annotated_flagged" from all samples to create Run_masterVar files.
##                     2) Added functionality to generate Run_AmpliconSummaryFinal_150X.txt and Run_AmpliconSummaryFinal_HEME_V2.3_150X.txt. Also, changed the respective codes to rename the regular Run_AmpliconSummaryFinal.txt and Run_AmpliconSummaryFinal_HEME_V2.3.txt as Run_AmpliconSummaryFinal_250X.txt and Run_AmpliconSummaryFinal_HEME_V2.3_250X.txt respectively.
##                     3) Added functionality to generate Run_MeanCovByPosition_HEMEao_V1.3.txt, Run_MeanCovByPositionFinal_HEMEao_V1.3.txt and Run_NormalizedMeanCovByPositionFinal_HEMEao_V1.3.txt.
##                     4) Added functionality to generate Run_MeanCovByPosition_BRCA(and mBRCA).txt, Run_MeanCovByPositionFinal_BRCA(and mBRCA).txt and Run_NormalizedMeanCovByPositionFinal_BRCA(and mBRCA).txt.
## Changes in V2.1:    1) Reverting back to Sample.canon_annotated_flagged from Sample.canon_annotated_flagged.HRR to in the concating step to form Run_masterVar files.
#!/bin/sh
...
then
	...
	if [ -e $i/$i.HEMEao_V1.3.MeanCovByPosition ];
	then
		cat $i/$i.HEMEao_V1.3.MeanCovByPosition >> Run_HEMEao_V1.3_MeanCovByPosition.txt
	elif [ -e $i/$i.BRCA.MeanCovByPosition ];
	then
			cat $i/$i.BRCA.MeanCovByPosition >> Run_BRCA_MeanCovByPosition.txt
	elif [ -e $i/$i.mBRCA.MeanCovByPosition ];
        then
                        cat $i/$i.mBRCA.MeanCovByPosition >> Run_mBRCA_MeanCovByPosition.txt
	fi
	...
	then
		cat $i/$i.Depth.below250X.amplicons.summary >> Run_AmpliconSummary_250X.txt
	fi
	if [ -s $i/$i.Depth.below150X.amplicons.summary ];
	then
		cat $i/$i.Depth.below150X.amplicons.summary >> Run_AmpliconSummary_150X.txt
	fi
...
then
	...
	then
		...
		then
			cat $i/$i.Depth.below250X.amplicons_sorted.summary >> Run_AmpliconSummary_HEME_V2.3_250X.txt
		fi
		if [ -s $i/$i.Depth.below150X.amplicons_sorted.summary ];
		then
			cat $i/$i.Depth.below150X.amplicons_sorted.summary >> Run_AmpliconSummary_HEME_V2.3_150X.txt
		fi
...
echo "Creating Final Amplicon Summary 250X File..\n"
sort -k 1,1 -r Run_AmpliconSummary_250X.txt|uniq > Run_AmpliconSummaryFinal_250X.txt

echo "Creating Final Amplicon Summary 150X File..\n"
sort -k 1,1 -r Run_AmpliconSummary_150X.txt|uniq > Run_AmpliconSummaryFinal_150X.txt
...
if [ -s Run_AmpliconSummary_HEME_V2.3_250X.txt ];
then
	echo "Creating Final Amplicon Summary 250X File for HEME_V2.3..\n"
	sort -k 1,1 -r Run_AmpliconSummary_HEME_V2.3_250X.txt|uniq > Run_AmpliconSummaryFinal_HEME_V2.3_250X.txt
fi

if [ -s Run_AmpliconSummary_HEME_V2.3_150X.txt ];
then
	echo "Creating Final Amplicon Summary 150X File for HEME_V2.3..\n"
	sort -k 1,1 -r Run_AmpliconSummary_HEME_V2.3_150X.txt|uniq > Run_AmpliconSummaryFinal_HEME_V2.3_150X.txt
fi
...
if [ -e Run_HEMEao_V1.3_MeanCovByPosition.txt ];
then
	sort -k 1,1 -r Run_HEMEao_V1.3_MeanCovByPosition.txt|uniq > Run_HEMEao_V1.3_MeanCovByPositionFinal.txt	
	python CombineStats.py Run_HEMEao_V1.3_MeanCovByPositionFinal.txt RunStatsFinal.txt Run_HEMEao_V1.3_NormalizedMeanCovByPosition.txt
fi
...
if [ -e Run_BRCA_MeanCovByPosition.txt ];
then
	sort -k 1,1 -r Run_BRCA_MeanCovByPosition.txt|uniq > Run_BRCA_MeanCovByPositionFinal.txt
	python CombineStats.py Run_BRCA_MeanCovByPositionFinal.txt RunStatsFinal.txt Run_BRCA_NormalizedMeanCovByPosition.txt
fi
if [ -e Run_mBRCA_MeanCovByPosition.txt ];
then
        sort -k 1,1 -r Run_mBRCA_MeanCovByPosition.txt|uniq > Run_mBRCA_MeanCovByPositionFinal.txt
        python CombineStats.py Run_mBRCA_MeanCovByPositionFinal.txt RunStatsFinal.txt Run_mBRCA_NormalizedMeanCovByPosition.txt
fi
