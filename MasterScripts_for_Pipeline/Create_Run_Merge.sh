...
##Changes in V2.2: The script now handles single HEME workflow.
##Changes in V2.2.1: 1) The 'CreateFinal.sh run' step has been silent.
##	             2) The CreateFinal.sh has been modified and hence its run-step is made active in this script.
RunFolderPath=$1
...
for i in `egrep -v '_skip|-skip|EGFRvIII|HEME_V1.2|HEMEao_V1.3' $RunFolderPath/SampleSheet_ToBeProcessed.txt|awk '{print $4}'|sort|uniq`
do
CountCat $i
done
...
then
	...
	then
		...
		bsub -J $HEME_merge_sample -n $Num_Threads -R "span[hosts=1]" -M $(($Max_Mem + 12288)) -o $RunFolderPath/$HEME_merge_sample/$HEME_merge_sample.PipeScript.out -e $RunFolderPath/$HEME_merge_sample/$HEME_merge_sample.PipeScript.err $RunFolderPath/$HEME_merge_sample/$HEME_merge_sample.PipeScript
