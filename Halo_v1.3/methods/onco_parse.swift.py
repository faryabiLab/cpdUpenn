    ...
    with open(anno, 'r') as anno_in:
        with open(anno_out, 'w+') as parsed:
            ...
            for line in anno_in:
                ...
                # Quick check for any massive errors
                if values[15] != "__UNKNOWN__":
                    continue
		...
		if transcript_strand == "+":
		    ...
		    strand="-1"
                ...
                rs_id = values[389] # dbsnp_147_RS << Updated
                rs_val = values[401] # dbsnp_147_VLD
                ...
                hgvs_c_change = values[258] # HGVS_coding_DNA_change
                hgvs_p_change = values[260] # HGVS_protein_change
                hgvs_g_change = values[259] # HGVS_genomic_change

                sift_pred = values[326] # dbNSFP_SIFT_pred
                clinvar_id = values[126] # ClinVar_rs
                hgnc_omimid = values[245] # HGNC_OMIM ID(supplied by NCBI)

                onekg_ac = values[82] # 1000gp3_AC
                onekg_an = values[86] # 1000gp3_AN
                onekg_af = values[83] # 1000gp3_AF
                onekg_amr = values[85] # 1000gp3_AMR_AF
                onekg_eur = values[93] # 1000gp3_EUR_AF
                onekg_afr = values[84] # 1000gp3_AFR_AF
                onekg_asn = values[91] # 1000gp3_EAS_AF
# Updated until here                
                ...
                #depth = values[406] depth_across_samples
                DP4 = values[128].split(',')
                try:
                    alt_depth = int(DP4[2]) + int(DP4[3]) # DP4 column [2]+[3]
                    ref_depth = int(DP4[0]) + int(DP4[1]) # DP4 column [0]+[1]
                except IndexError:
                    alt_depth = float(values[79])
                    ref_depth = float(values[80])

                depth = alt_depth + ref_depth

                temp_vaf = float(values[271])*100 # allele_frequency
                if temp_vaf == 50 or temp_vaf == 100:
                    vaf = ((float(values[79]))/(float(values[79])+float(values[80])))*100
                else:
                    vaf = temp_vaf

                qual = values[425] # qual
