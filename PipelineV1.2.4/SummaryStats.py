...
if(os.path.exists(FileName) and os.path.exists(FileName2) and os.path.exists(FileName3) and os.path.exists(FileName4) and os.path.exists(AmpliconFile) and os.path.exists(ClipStatFile) and os.path.exists(BtrimStatsR1) and os.path.exists(BtrimStatsR2) and os.path.exists(SignatureID_File)):
	...
	BtrimSubProcessR1=subprocess.Popen(['grep',"Total sequences",BtrimStatsR1],stdout=subprocess.PIPE).communicate()[0].rstrip()
	BtrimSubProcessR2=subprocess.Popen(['grep',"Total sequences",BtrimStatsR2],stdout=subprocess.PIPE).communicate()[0].rstrip()
	NumR1FastqReads=BtrimSubProcessR1.split(" ")[2]
	NumR2FastqReads=BtrimSubProcessR2.split(" ")[2]
	...
	if(TotalStartingReads!=0):
		...
		OutString=("\t"+"0")*18      ## Assigning value "0" to all fields in SummaryStats file EXCEPT the SampleName field
