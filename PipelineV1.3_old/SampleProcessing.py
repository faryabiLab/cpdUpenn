...
## This version calls insertion and deletions on pre filtered bam files, removes all variants less than or equal to 6bp then merges those with the gatk results later. 
## this Version also removes more of the intermediate files:  *.sam, *.bam, *.btrim.summary, *fastq (uncommressed), *.pe (used in btrim)

## The Version V1.3 includes the following functionalities:
## 1) Calculates Indel Depths 2) Calculates a Genotype Signature for a sample and includes it in the final stats 3) Flags Adjacent Snps in the sample's final cannon_annotated variant file to give cannon_annotated_flagged file which is also moved to the main folder at the end of the sample's pipe.
## 4) the 'HRunRemoved' has been replaced by 'HRR' in the filenames. 5) The onTarget Bam is now moved to the main folder instead of being deleted at the end. The onTarget Bam Index is also moved to the main folder.
## 6) Garbage Picker is included in this version which picks up the amplicon-specific reads and gives the counts of mapped and unmapped reads for each amplicon.
...
for eachsample in Samples:
	...
	eParam="140"
    	...
    	eParam="190"
    PrimerPartsFile=PrimersFile.replace(".txt",".primParts")
    PrimerRCPartsFile=PrimersRCFile.replace(".txt",".primParts")
    PrimerManifestFile=Description+".manifest"
...
# Phase 1 - Mapping, Filtering, Clipping and Stats Generation  
    ...
    PipelineScript_FILE.write("echo 'Running Garbage Picker..\n\n'"+"\n")
    PipelineScript_FILE.write("./garbPick.sh "+SampleName+"/"+SampleName+"_R1.btrim.summary.txt "+SampleName+"/"+SampleName+"_R2.btrim.summary.txt "+PrimerPartsFile+" "+PrimerRCPartsFile+" "+PrimerManifestFile+"\n")
    
    
    	
    PipelineScript_FILE.write("echo 'Carrying out Alignment of fastq files with hg19 reference genome using Novoalign...\n\n'"+"\n")
    PipelineScript_FILE.write("/home/bdaber01/novocraft/novoalign -d /Drive_D/Databases/genomes/hg19_genome.nix -f "+SampleName+"/"+SampleName+"_"+SampleCount+"_L001_R1_001.btrim.fastq.pe "+SampleName+"/"+SampleName+"_"+SampleCount+"_L001_R2_001.btrim.fastq.pe -i PE "+FragSize+" -c 6 -n 200 -o FullNW -o SAM > "+SampleName+"/"+SampleName+".novo.sam" +"\n")
    ...
    PipelineScript_FILE.write("echo 'Making CoverageSummary for the sample..\n\n'"+"\n")
...
#Phase 2 - Variant Calling and Merging of All Stats
    ...
    PipelineScript_FILE.write("java -Xmx3G -jar /home/bdaber01/GenomeAnalysisTK-1.6-13-g91f02df/GenomeAnalysisTK.jar -R /home/bdaber01/Drive_D/Databases/genomes/hg19_genome.fa -T UnifiedGenotyper -I "+SampleName+"/"+SampleName+".novo.sorted.fixed.ontarget.afterFilter.clipped.bam --dbsnp /home/bdaber01/Drive_D/Databases/dbsnp/hg19_dbsnp137.vcf -o "+SampleName+"/"+SampleName+".GATK.af4percent.vcf -nt 4 -glm SNP -mbq 15 -dt NONE -alleles:VCF "+SampleName+"/"+SampleName+".GenotypeGivenAlleles.sorted.vcf -gt_mode GENOTYPE_GIVEN_ALLELES -out_mode EMIT_ALL_SITES -stand_emit_conf 10.0 -L "+BED_FILE + "\n")
    ...
    #PipelineScript_FILE.write("java -jar /home/bdaber01/snpEff_3_0/snpEff.jar -c /home/bdaber01/snpEff_3_0/snpEff.config hg19 -i vcf -o txt -noStats "+SampleName+"/"+SampleName+".combined.filtered.HRR.vcf > "+SampleName+"/"+SampleName+".combined.filtered.HRR.snpeff.txt"+"\n")
    ...
    PipelineScript_FILE.write("echo 'Running MultiAllelic.py script on the filtered.HRR.vcf to convert all mutiallelic position to single allelic ones..\n\n'"+"\n")
    ...
    PipelineScript_FILE.write("perl /home/bdaber01/annovar/convert2annovar.pl  -format vcf4 -includeinfo "+SampleName+"/"+SampleName+".combined.filtered.HRR.multiallelic.vcf > "+SampleName+"/"+SampleName+".combined.filtered.HRR.multiallelic.avinput"+"\n")
    ...
    PipelineScript_FILE.write("java -Xmx3G -jar /home/bdaber01/GenomeAnalysisTK-1.6-13-g91f02df/GenomeAnalysisTK.jar -R /home/bdaber01/Drive_D/Databases/genomes/hg19_genome.fa -T UnifiedGenotyper -I "+SampleName+"/"+SampleName+".novo.sorted.fixed.ontarget.afterFilter.clipped.bam --dbsnp /home/bdaber01/Drive_D/Databases/dbsnp/hg19_dbsnp137.vcf -o "+SampleName+"/"+SampleName+".signatureGenotype.vcf -nt 4 -glm SNP -mbq 15 -dt NONE -alleles:VCF /Drive_D/Databases/sigIDPanel/SignatureIDPanel."+Description+".sorted.vcf -gt_mode GENOTYPE_GIVEN_ALLELES -out_mode EMIT_ALL_SITES -stand_emit_conf 10.0 -L "+BED_FILE + "\n")
    PipelineScript_FILE.write("echo 'Creating the signatureID file for sample..\n\n'"+"\n")
    PipelineScript_FILE.write("python genotypeToSig.py "+SampleName+"/"+SampleName+".signatureGenotype.vcf "+SampleName+"/"+SampleName+".signatureID"+"\n")
    ...
    PipelineScript_FILE.write("mv "+SampleName+"/"+SampleName+".canon_annotated_flagged "+"."+"\n")
    PipelineScript_FILE.write("mv "+SampleName+"/"+SampleName+".novo.sorted.fixed.ontarget.ba* "+"."+"\n")
