...
## Changes in V2.1.1: 1) The script now copies all data from MiSeqAnalysis (instead of MiSeqOutput) RunFolder to local Drive_D of Comp1. 2) The end step of copying the MiSeqOutput RunFolder to isilon (i.e. /MSdata/illumina/MSout) is made silent as that RunFolder would already be present on Isilon according to the new workflow where MiSeqOutput RunFolder gets written to Isilon (i.e. /MSdata/illumina/SeqOut) in real time. 3) Changed 'WaitTillComplete.sh' to 'WaitTillComplete_new.sh'. Changed respective echo statement.
if [ $# -ne 4 ];
then
	...
	echo "Running WaitTillComplete_new.sh.."

	## Waiting Till MiSeq Analysis gets completed

	./WaitTillComplete_new.sh $MiSeqAnalysisFolder
	...
	## Copying Data from the Run Folder on MiSeqAnalysis to the Run Folder on Local Drive D

	echo "Now Copying the Data from Run Folder on MiSeqAnalysis to the Run Folder which would be created on Local Drive D.."
	
 	echo "Making Run Folder inside /Drive_D/illumina/Analysis  .."

	mkdir $localFolder_Comp1
	cp $MiSeqAnalysisFolder/Data/Intensities/BaseCalls/*.fastq.gz $localFolder_Comp1
	cp $MiSeqAnalysisFolder/SampleSheet.csv $localFolder_Comp1
