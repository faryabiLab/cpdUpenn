...
##Change in V2.0 : If there is no cDNA change annotated to an exonic variant then the corresponding column is remained empty in the final output (i.e. '.refseq file'). Basically adding "try,except" commands to handle cChange appropriately. Previously, these commands were used to handle only pChange.
