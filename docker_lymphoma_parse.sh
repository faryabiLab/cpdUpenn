...
exon_bed='/project/cpdlab/Lymphoma/final.bed' 
...
do
  ...
  # Fix sample name
  samp_name=$(echo $file | sed 's/\..*//' | sort -t _ -u -k 1,1 -k 2.2n)
  full_name=$(awk -v var1=$(basename -- "$samp_name") 'BEGIN {FS=","} $2==var1' ${vcfs}/SampleSheet.csv | awk -v var2="A" 'BEGIN {FS=","} $3==var2' | awk -v var3="1" 'BEGIN {FS=","} $7==var3 {print $9}')
  full_name2=$(echo $full_name | sed "s///")
  python ${onco_parse} $file.anno.TCGAMAF $file.parsed.tsv ${full_name2}"-"${dir}
  echo $file" becomes "${full_name2}"-SEQ-180248"
done
...
cd ${vcfs}
count=0
declare -a array=(A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B A B) # A/B marker (Manifest column in SampleSheet)
...
do
  cd FASTQs/
  total_reads=$(zcat ${line}*fastq.gz | echo $((`wc -l`/4)))
  ...
  true_name=`cut -d'_' -f1 <<< ${line}` # Sample name value in SampleSheet
  nameID=${true_name}${array[count]} # Sample ID value in SampleSheet
  name=$(awk -v var1=$true_name 'BEGIN {FS=","} $2==var1' SampleSheet.csv | awk -v var2=${array[count]} 'BEGIN {FS=","} $3==var2' | awk -v var3="1" 'BEGIN {FS=","} $7==var3 {print $9}')
  echo "Collecting statistics for "$(echo $name | sed "s///")${array[count]}
    
  if [[ ${array[count]} == A ]];
  then
    sum=0 
    reads1=$(cut AmpliconCoverage_M1.tsv -f `head -1 AmpliconCoverage_M1.tsv | tr "\t" "\n" | grep -n "^$nameID"'$' | cut -f 1 -d :`| awk '{if (NR!=1) {print}}')
    for num in ${reads1};
    do
      sum=$((sum+num))
    done
...
#  samtools depth $line.bam > $line.coverage.txt
  ...
  bedtools genomecov -dz -ibam $line.int.bam > $line.coverage.txt
  ...
  ex_counter250=`cat $line.int.bedgraph | awk '$4<250' | bedtools merge -i stdin | wc -l`
  ex_counter150=`cat $line.int.bedgraph | awk '$4<150' | bedtools merge -i stdin | wc -l`
  #cat $line.int.bedgraph | awk '$4<250' | awk '{print $1"\t"$2"\t"$3}' > ${line}.250fil.bedgraph
  #ex_counter250=`bedtools merge -i ${line}.250fil.bedgraph | wc -l`

  #cat $line.int.bedgraph | awk '$4<150' | awk '{print $1"\t"$2"\t"$3}' > ${line}.150fil.bedgraph
  #ex_counter150=`bedtools merge -i ${line}.150fil.bedgraph | wc -l`

  clip_count="NA"
  
  echo -e $(echo $name | sed "s///")-"$dir\t$total_reads\t$total_aligned_reads\t$per_lost\t$mapped_reads\t$per_mapped\t$on_target_reads\t$per_on_target\t$qual_filter_reads\t$per_after_qual\t$per_usable\t$mean_cov\t$per_0\t$per_1\t$per_250\t$per_1K\t$ex_counter250\t$clip_count\t$ex_counter150" >> ${stats}
  
  # Long one-liner for dummy line variant insertion here, perhaps? Something like...
  echo -e $(echo $name | sed "s///")-"$dir\tchr1\t1\t-\t-\t-\tNA\t0\t0\tNA\t0\tXXXX\tNA\tXXXX\tNA\t\tXXXX\tNA\t\tNA\tNA\tNA\tNA\tNA\t\tc.-\t\tXXXX\tXXXX\tXXXX\tXXXX\t\t\tchr1\t1\t\t-\t-\t0\t\tNA\tNA\tNA\t\t\t\t\t\t\t\tXXX\t0\t0\t0\t0\tXXXX\t0\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\t0\t0\tXXXX\t\t\tXXXX\t\tNA\t\t\t0\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\t\t\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\tNA\t\tNA\tNA\tNA\tNA\tNA\tNA" >> ../${variants}
  mv $line.bam $name"-"$dir.bam
  mv $line.bam.bai $name"-"$dir.bam.bai

  cat $line.int.bedgraph | awk '$4<250' | bedtools merge -i stdin > $line.exonic_reg_bel250.bed
  count=$((count+1))
done
cd ..
...
rm *int.bam
rm -rf FASTQs/
for file in *bam*; do mv $file $(echo $file | sed "s///"); done
cd ..
