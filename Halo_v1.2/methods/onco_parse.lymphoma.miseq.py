...
# onco parse
def onco_parse(anno,anno_out,seq_id,gene_list):
    with open(anno, 'r') as anno_in:
        with open(anno_out, 'w+') as parsed:
            ...
            for line in anno_in:
                ...
                onc_type = values[9]
                sample_name = seq_id # change to values[16]+"-"+seq_id as needed
                rs_id = values[391] # Updated
                rs_val = values[403] # Updated
                ...
                hgvs_c_change = values[256]
                hgvs_p_change = values[258]
                hgvs_g_change = values[257] # Updated

                sift_pred = values[328]
                clinvar_id = values[125]
                hgnc_omimid = values[243]
                ...
                snpeff_ann = values[106]
                #snpeff_split = snpeff_ann.split('|')
                snpeff_split = snpeff_ann.split(',')
                for ann in snpeff_split:
                    print ann
                    ann_split = ann.split('|')
                    if ann_split[3] in gene_list:# matches any genes in the list
                        snpeff_transcript = ann_split[6]
                        snpeff_gene = ann_split[3]
                        snpeff_coding = ann_split[7]
                        snpeff_effect = ann_split[1]
                    else:
                        continue
                    print " "  
                #snpeff_effect= values[128] # snpeff_split[1] returning wrong effect occasionally
                #snpeff_transcript = snpeff_split[6]
                #snpeff_gene = snpeff_split[3] # returning HUGO symbol here instead
                #snpeff_coding = snpeff_split[7]
                if (len(ann_split[8]) > 0):
                    snpeff_rank = ann_split[8]
                    snpeff_rank = snpeff_rank.split('/')
                    snpeff_rank = snpeff_rank[0]
                else:
                    snpeff_rank = ""
                snpeff_c_change = ann_split[9]
                snpeff_p_change = ann_split[10]
                ...
                depth = values[408] # depth_across_samples
                alt_depth = values[79] # t_alt_count
                ref_depth = values[80] # t-ref_count
                depth = int(alt_depth) + int(ref_depth) # math
                vaf = vaf_percent(alt_depth, depth)
                qual = values[419] # qual

                # Extract probe bias, strand bias, etc. data from/to 'FILTER' column
                PB = values[266]
                R8 = values[268]
                SB = values[269]
                LowDP = values[260]
                LowGQ = values[261]
                LowVariantFreq = values[262]
                ...
                if LowVariantFreq == "PASS":
                    FILTER=FILTER
                elif LowVariantFreq == "FAIL":
                    FILTER=FILTER+"LowVariantFreq"
                ...
                #check for pindel
               # if (len(values[358]) > 0):
               #     alt_depth = values[358]
               #     ref_depth = values[329]
               #     depth = int(alt_depth) + int(ref_depth)
               #     vaf = vaf_percent(alt_depth, depth)
               #     qual = "NA"
                
                # These cutoffs filter out low-depth reads from the NTC. They may be changed in the future. 
                if int(depth) < 50 :
                    print str(depth)+" depth below 50"
                    continue
                if alt_depth < 5 :
                    print str(start_pos+ " alt_depth ")
                    continue
                if float(vaf) < 5.0:
                    print str(start_pos + " vaf")
                    continue
                print sample_name+'\t'+FILTER
                parsed.write(sample_name + '\t' + chrom + '\t' + start_pos + '\t' + ref + '\t' + alt + '\t' + onc_type + '\t' + 'NA' + '\t' + qual + '\t' +
                             str(depth) + '\t' + 'NA' + '\t' + geneid + '\t' + gene + '\t' + 'NA' + '\t' + snpeff_transcript + '\t' + 'NA' + '\t' + snpeff_rank +
                             ...
                             '\t' + FILTER + '\t' + 'NA' + '\t' + 'NA' + '\t' + 'NA' + '\t' + onekg_ac + '\t' + onekg_an + '\t' + onekg_af + '\t' + onekg_amr + '\t' +
...
def main():
    ...
    anno_out=sys.argv[2]
    seq_id=sys.argv[3]
    with open('/project/cpdlab/Lymphoma/lymphoma_target_list.txt','r') as myfile:
        gene_list = myfile.read()
    onco_parse(anno,anno_out,seq_id,gene_list)
