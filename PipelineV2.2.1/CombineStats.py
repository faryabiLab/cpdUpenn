...
##Changes in V2.1.1_hpc: 1) The "split("_")" function which was used to split the 'iLineCompo[0]' to give the value for 'iLineSample' has been removed. This is because the 'iLineCompo[0]' now itself is equal to the sample-name, i.e. 'iLineSample'.
##			 2) The calculation has been changed a bit for the formation of "iLine" at the end of each instance of for loop. The "iLine" now has the calculation: "iLineSample+"\t"+"\t".join(iLineCompo[1:])" instead of the calculation:""\t".join(iLineCompo[0:])". This was done to write the sample-names more cleanly (i.e. not containing "_mean_cvg" in them) in the final output file.
...
for iLine in infile1Lines:
	...
	if(iLineSample=="Target"):
		...
		iLine=iLineSample+"\t"+"\t".join(iLineCompo[1:])
		outfile.write(iLine+"\n")
