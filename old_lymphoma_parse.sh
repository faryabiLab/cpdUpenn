...
onco_parse='/project/cpdlab/Scripts/SolidV2/solidv2_v1.2/methods/onco_parse.lymphoma.miseq.py'
...
do
  ...
  # Remove the three lines below for future runs, and edit the 'sample_name' definition in onco_parse
  # ---------------------------------------------------------
  samp_name=$(echo $file | sed 's/\..*//' | sort -t _ -u -k 1,1 -k 2.2n)
  full_name=$(awk -v var1=$(basename -- "$samp_name") 'BEGIN {FS=","} $2==var1' ${vcfs}/SampleSheet.csv | awk -v var2="A" 'BEGIN {FS=","} $9==var2 {print $1}')
